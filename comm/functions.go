...
// 将unix时间戳格式化为yyyymmdd H:i:s 格式字符串
...
// 对字符串进行签名
func CreateSign(s string) string {
	s += string(conf.SignSecret)
...
// 从map[string]interface{}中得到指定的value 并转为int64
...
// 从map[string]interface{}中得到指定的 value, 转为string
...
// 从map[string][string[中得到指定的value
